package com.ias.utils.data.checks;
...
public class PersistenceChecks {
    ...
    public static <T> void checkNotFoundStatus (T result, Class<T> resource){
        if (Objects.isNull(result)) {
            throw new EntityNotFoundException(resource.getTypeName() + ": No se encontro ninguno");
        }
    }
    ...
    public static <T> void isIdConsistentForUpdate(boolean consistent, Class<T> resource){
        if (!consistent) {
            throw new BadRequestException(resource.getTypeName() + ": no se puede actualizar los idenficadores no son consistentes");
        }
    }
